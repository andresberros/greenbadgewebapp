...
import Quagga from 'quagga';
  ...
  class Scanner extends Component {
    ...
    <input type="button" id="btn" value="1_Start/Stop the scanner"   onClick={(event) => this.handleClick(event)}/>
      </div>
        );
    }

    handleClick(event){
      if (this.state.scannerRunning) {
        Quagga.stop();
    } else {
    ...
    startScanner() {
      Quagga.init({
        ...
        this.setState({scannerRunning :false})
    });
